...
class LAI_S1S2_fusion() :
	...
	def __init__(self,jsonloc, FNAME, workingdir, s1dir, s2dir, ECMWF_ERA5_VPD, snap_graphs_dir, snap_gtp_dir, startdate, enddate): 
	...
	--------------------------------------
		...
		os.chdir('%s/%s/' %(self.workingdir,self.s1dir))
		folds = glob.glob('*')
		...
		for i in range(len(folds)):
			...
			<node id="Write">
				...
				<parameters class="com.bc.ceres.binding.dom.XppDomElement">
					<file>%s/processed/Subset_%s_Orb_NR_Cal_ML_TC_dB.tif</file>
			...
			""" %('%s/%s'%(self.workingdir,self.s1dir),folds[i],poly,'%s/%s'%(self.workingdir,self.s1dir),folds[i][:-4])
	...
	def S2_to_LAI() : 
		...
		*** The final LAI maps' spatial resolution can be adjusted by changing the <targetResolution> (line 332) -- currently 40m 
		...
		fieldpolygonloc = gpd.read_file(self.jsonloc) 
		poly = str(fieldpolygonloc.geometry.iloc[0])
		os.chdir("%s/%s" %(self.workingdir,self.s2_data_dir))
		...
		for i in range(len(folds)):
			...
			""" %(s2_data_dir,folds[i],'%s/%s'%(self.workingdir,self.s2_data_dir),folds[i][:-4])
		...
		os.chdir("%s/%s/processed" %(self.workingdir,self.s2_data_dir))
		folders = []
	...
	def lai_ts_creation() : 
		...
		os.chdir('%s/%s/processed' %(self.workingdir,self.s2dir))
		folders_S2 = []
		for file in glob.glob("*_p2.tif") : folders_S2.append(file)
		folders_S2.sort()
		...
		os.chdir('%s/%s/processed' %(self.workingdir,self.s1dir))
		folders_S1 = []
		...
		np.save("%s/%s_RF_R2=%s.npy" %(self.workingdir,self.FNAME,RF_score),np.array([RF_score])) # empty .npy with the RF model's R2 as its name 
		...
		daily_rfLAI = pd.DataFrame(columns={'emptycol'},index=pd.date_range(start=self.startdate, end=self.enddate, freq='D'))
		...
		LAI_timeseries = daily_rfLAI.rf_LAI.resample('7D',label='right').first()[:-1] # the LAI value returned is the first for each week within the examined period

		# Save the RF predicted weekly LAI time-series
		np.save(("%s/LAI_timeseries_%s.npy" %(self.workingdir,self.FNAME)),LAI_timeseries) 
